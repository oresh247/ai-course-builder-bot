...
MODULE_CONTENT_SYSTEM_PROMPT = """Ты — эксперт по созданию образовательного контента для IT-курсов.
...
- Слайды должны быть лаконичными (3-5 пунктов на слайд)
- Для code слайдов обязательно добавляй примеры кода в code_example
- Для всех слайдов добавь notes - заметки для преподавателя
    ...
    {{
      ...
      "slides": [
        {{
          ...
          "content": "Текст слайда (3-5 пунктов через \\n)",
          "slide_type": "title",
          "code_example": null,
          "notes": "Заметки для преподавателя о том, как подать материал"
        }},
        {{
          "slide_number": 2,
          "title": "Пример кода",
          "content": "Описание примера",
          "slide_type": "code",
          "code_example": "# Код с комментариями\\nprint('Hello')",
          "notes": "Объясните построчно"
        }}
      ]
    }}
  ]
...
# ============================================================================
# ПРОМПТ ДЛЯ ПЕРЕГЕНЕРАЦИИ ЛЕКЦИИ (LECTURE)
# ============================================================================

LECTURE_REGENERATION_SYSTEM_PROMPT = """Ты — эксперт по созданию образовательного контента для IT-курсов.
Улучшаешь и дорабатываешь лекции со слайдами.
Отвечаешь строго в JSON формате."""

LECTURE_REGENERATION_PROMPT_TEMPLATE = """Перегенерируй ДЕТАЛЬНУЮ лекцию для IT-курса в формате слайдов.

КОНТЕКСТ:
Курс: {course_title}
Аудитория: {target_audience}
Модуль: {module_title}
Лекция: {lecture_title}
Текущее количество слайдов: {num_slides}
{custom_requirements}
ЗАДАЧА:
Создай улучшенную версию этой лекции с {num_slides} слайдами. Сохрани тему и структуру, но улучши содержание, примеры и объяснения.

ТИПЫ СЛАЙДОВ:
- title: Заглавный
- content: Теория (3-5 пунктов)
- code: Примеры кода (обязательно заполни code_example)
- diagram: Схемы (описание визуализации)
- quiz: Вопросы для проверки
- summary: Итоги

ТРЕБОВАНИЯ:
- Сохрани общую структуру и количество слайдов
- Улучши формулировки и примеры
- Добавь больше деталей в code_example для code слайдов
- Улучши notes для преподавателя
- Учитывай уровень аудитории
...
{{
  ...
  "learning_objectives": [
    "конкретная цель обучения 1",
    "конкретная цель обучения 2",
    "конкретная цель обучения 3"
  ],
  "key_takeaways": [
    "ключевой вывод 1",
    "ключевой вывод 2",
...
# ============================================================================
# ПРОМПТ ДЛЯ ПЕРЕГЕНЕРАЦИИ СЛАЙДА (SLIDE)
# ============================================================================

SLIDE_REGENERATION_SYSTEM_PROMPT = """Ты — эксперт по созданию образовательных слайдов для IT-курсов.
Улучшаешь и дорабатываешь отдельные слайды.
Отвечаешь строго в JSON формате."""

SLIDE_REGENERATION_PROMPT_TEMPLATE = """Перегенерируй ОДИН слайд для IT-лекции.
...
{custom_requirements}
ЗАДАЧА: 
Создай улучшенную версию этого слайда. Сохрани тему, но улучши содержание, примеры и формулировки.

ТИПЫ СЛАЙДОВ:
- title: Заглавный слайд
- content: Теория (3-5 пунктов)
...
ТРЕБОВАНИЯ:
- Контент должен быть лаконичным (3-5 пунктов)
- Для code слайдов обязательно заполни code_example с комментариями
- Добавь полезные notes для преподавателя
- Учитывай уровень аудитории ({target_audience})
...
{{
  ...
  "notes": "заметки для преподавателя о том, как подать этот слайд"
}}
...
# ============================================================================
# ПРОМПТ ДЛЯ ГЕНЕРАЦИИ ОПИСАНИЯ МОДУЛЯ
# ============================================================================

MODULE_GOAL_GENERATION_SYSTEM_PROMPT = """Ты — эксперт по созданию образовательных программ.
Создаёшь чёткие и измеримые цели для модулей курса."""

MODULE_GOAL_GENERATION_PROMPT_TEMPLATE = """Создай детальную цель для модуля IT-курса.
...
- 2-3 предложения
- Опиши, что студент узнает и сможет сделать после модуля
- Учитывай уровень аудитории

Верни ТОЛЬКО текст цели, без дополнительных комментариев."""
...
def format_lessons_list(lessons):
    ...
    for i, lesson in enumerate(lessons, 1):
        result += f"{i}. {lesson.lesson_title} ({lesson.format}, {lesson.estimated_time_minutes} мин)\n"
        result += f"   Цель: {lesson.lesson_goal}\n"
    return result
